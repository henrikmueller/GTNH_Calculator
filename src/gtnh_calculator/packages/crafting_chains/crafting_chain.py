...
class CraftingChain:
    ...
    def print(self, time_factor, time_interval: str) -> CraftingChainStatistics:
        """
        ----------------------------------------------------------------------------------------------------------------
        ...
        data[:, 0] = [machine_names[i] for i in self.recipe_indices]
        data[:, 1] = [self.recipes[i].input_string(time_factor * self.recipe_amounts[i]) for i in self.recipe_indices]
        data[:, 2] = [self.recipes[i].output_string(time_factor * self.recipe_amounts[i]) for i in self.recipe_indices]
        data[:, 3] = eu_per_tick
    ...
    # def draw_old(self, materials: Dict[int, Material], recipes: Dict[int, Recipe], time, time_factor,
