...
class CraftingChainFinder:
    ...
    def draw_optimal_crafting_chain(
            ...
            excluded_recipes: set[int] | None = None
    ) -> None:
        """
        :param inputs:
        :param output_spec:
        ...
        output_material, output_amount = output_spec

        recipe_indices = np.array([recipe.id for recipe in self.recipes.values() if recipe.id not in excluded_recipes])

        materials = list(self.material_list.materials_by_name.values())
        p, q = len(materials), recipe_indices.size
        ...
        recipe_vector = np.zeros(len(self.recipes))
        for i, entry in enumerate(result.x):
            recipe_vector[recipe_indices[i]] = entry
    ...
    def output_maximizer(
            self,
            input_spec: tuple[Material, float],
            weights: Dict[Material, float],
            time: str,
            time_interval: str,
            recipe_weight_factor=1,
            infinite_materials: set[Material] | None = None,
            excluded_materials: set[Material] | None = None,
            excluded_recipes: set[int] | None = None
    ) -> None:
        """
        ...
        time, _ = time_to_seconds(time)
        time_interval, time_interval_name = time_to_seconds(time_interval)
        input_material, input_amount = input_spec
        ...
        # If a material is specified as an input: Remove it as output from every recipe.
        X[input_material.id][X[input_material.id] > 0] = 0
        ...
        # TODO: A_ub still correct when recipe_indices is not full?
