...
def material_exploration(
    ...
    for i, material in enumerate(matching_materials[:max_displayed_materials]):
        with cols[i % number_of_columns]:
            material_card_crafting_chain_database(material)
...
def recipe_exploration_display(
    ...
    for recipe_row in df_filtered.itertuples(index=False):
        ...
        displayed_recipes += crafting_chain_database.instantiate(recipe)
    ...
    changed_instantiated_recipe_ids = []
    if len(displayed_recipes) > 0:
        for instantiated_recipe in displayed_recipes:
            with st.container(border=True):
                a, b = st.columns(2)
                with a:
                    instantiated_recipe_id = adapt_crafting_chain_recipe(
                        instantiated_recipe, database.machine_options_book)
                    if instantiated_recipe_id is not None:
                        changed_instantiated_recipe_ids.append(instantiated_recipe_id)
                with b:
                    display_crafting_chain_recipe(instantiated_recipe)
    return changed_instantiated_recipe_ids
...
def recipe_exploration(
    ...
    st.markdown('### Explore Recipes:')
    recipe_exploration_filters = get_recipe_exploration_filters(database, all_materials)
    df_filtered = apply_recipe_exploration_filters(
        database=database,
        crafting_chain_database=crafting_chain_database,
        recipe_exploration_filters=recipe_exploration_filters
    )
    changed_instantiated_recipe_ids = recipe_exploration_display(
        database=database,
        crafting_chain_database=crafting_chain_database,
        df_filtered=df_filtered,
        max_displayed_recipes=max_displayed_recipes
    )
    print(f'Changed instantiated recipe IDs: {changed_instantiated_recipe_ids}')


def material_card_crafting_chain_database(material: Material):
