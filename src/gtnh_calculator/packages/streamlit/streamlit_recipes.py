...
from packages.factory_database.factories import Factory
...
def display_recipe_environment(
    instantiated_recipe: InstantiatedRecipe, recipe_environments_state: RecipeEnvironmentsState
) -> None:
    """
    ...
    with aa:
        ...
        with d:
            st.markdown(f'#### {selected_recipe_environment.machine.machine_name_specified}')
        ...
        if len(valid_machines) > 1:
            with st.expander('Other valid machines', width=400):
                for machine in valid_machines:
                    if machine == selected_recipe_environment.machine:
                        continue
                    a, b = st.columns([0.5, 5], gap='small')
                    with a:
                        material_image(machine.item)
                    with b:
                        st.markdown(f'{machine.machine_name_specified}')
        ...
        st.markdown(f'**Valid voltage tiers:** {", ".join(VoltageTier.voltage_tier_name(v) for v in valid_voltage_tiers)}')
    with bb: 
        if len(instantiated_recipe.machine_options.valid_options) > 0:
...
def adapt_crafting_chain_recipe(
    ...
    with aa:
        ...
        def clicked_enable_toggle():
            ...
            recipe_environments_state.set_enabled(id, new_value)

        if enabled_checkbox:
            st.checkbox(
                label="Enabled" if recipe_environments_state.is_enabled(id) else "Disabled", 
                key=f"enable_{id}_{key_suffix}",
                value=recipe_environments_state.is_enabled(id),
...
def display_factory(
    factory: Factory, recipe_headline: str = ''
) -> None:
    color = RGBAColor((0.0, 0.0, 0.0, 0.9))
    html = Template(dedent("""
    <style>
    ...
    try:
        img_base64 = get_base64_image(f'db/images/{factory.icon_path}')
        machines_html = f"""<div class="tooltip">
            <img src="data:image/png;base64,{img_base64}" width="36">
            <span style="margin-left:6px;">{recipe_headline}</span>
            <span class="tooltiptext">{factory.factory_name}</span>
        </div>
        """
    except FileNotFoundError as e:
        machines_html = ''
    ...
    inputs_html = ''
    for i, (input, amount) in enumerate(factory.inputs.items()):
        try:
    ...
    outputs_html = ''
    for i, (output, amount) in enumerate(factory.outputs.items()):
        try:
