...
from packages.streamlit.streamlit_logic import (
    display_example_files, file_selection
)
from packages.streamlit.crafting_chain_database_exploration import (
    material_exploration, recipe_exploration
)
...
display_example_files()
uploaded_file = file_selection()
...
class CraftingChainCalculationDisplay:
    ...
    def __post_init__(self):
        if 'update_optimization_result' not in st.session_state:
            st.session_state['update_optimization_result'] = True
    ...
    def display_page(self) -> None:
        ...
        if st.button('Update Optimization'):
            self.set_update_optimization_result(True)
    ...
    def update_optimization_result(self) -> bool:
        return st.session_state['update_optimization_result']

    def set_update_optimization_result(self, value: bool) -> None:
        st.session_state['update_optimization_result'] = value
    ...
    def prepare_optimization(self) -> bool:
        with st.spinner('Fitting recipes to machines...', show_time=True):
            ...
            self.instantiated_recipes = self.crafting_chain_database.instantiate_all(df_recipes)
        ...
        successful_preparation = not outputs_missing
    ...
    def calculate_crafting_chain(self) -> None:
        ...
        with st.spinner('Calculating optimal crafting chain...', show_time=True):
            ...
            if 'crafting_chain' not in st.session_state or self.update_optimization_result():
            ...
            self.crafting_chain = st.session_state['crafting_chain']
            self.set_update_optimization_result(False)
    ...
    def display_optimization_results(self) -> None:
        if self.crafting_chain is None:
            _LOGGER.warning('Crafting chain is None. Cannot display crafting chain.')
            return
        ...
        unspecified_machines = set().union(*[r.valid_machines for r in self.crafting_chain.recipe_amounts.keys() if r.machine.unspecified])
...
{'\n'.join([f'- {m.__str__()}\n' for m in unspecified_machines])}
    ...
    def display_looping_materials(self) -> None:
        ...
        looping_materials = (
            ...
            set().union(*[set(r.get_outputs()) for r, a in self.crafting_chain.recipe_amounts.items() if a > 0])
        )
